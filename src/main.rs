...
fn main() -> ExitCode {
    ...
    if args.len() == 1 {
        ...
        println!(
            "Optionally including '--validate-slow' in the argument list will skip SIMD calculation, typically just for testing."
        );
    ...
    if "--file" == input_type {
        ...
        if args.len() == 3 {
            let result = calculate_crc_64_simd_from_file(file);
            let result_bytes = result.to_be_bytes();

            println!("Decimal: \t{}", result);
            println!("Binary: \t{:b}", result);
            println!(
                "Base64: \t{}",
